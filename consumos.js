// ===============================================
// CONSUMOS MENSUALES - v2 (limpio) - FIX TZ
// ===============================================

// Mes actual visible en consumos (formato "YYYY-MM")
// Asegúrate de inicializarlo en algún punto de tu app, por ejemplo:
// let currentConsumosMes = new Date().toISOString().slice(0, 7);

// ------------------------------------------------
// Helper robusto para mover meses sin bug de timezone
// ------------------------------------------------
function addMonthsYm(ym, delta) {
  const [y, m] = ym.split('-').map(Number); // m = 1..12
  const d = new Date(Date.UTC(y, m - 1, 1)); // 1º día del mes en UTC
...
function consumosPrev() {
  currentConsumosMes = addMonthsYm(currentConsumosMes, -1);
...
function renderConsumos() {
  ...
  const mesLabel = new Date(year, month - 1, 1).toLocaleDateString('es-ES', {
    month: 'long',
    year: 'numeric'
  });
...
// ------------------------------------------------
// Renderizar gráfico de tendencia (FIX TZ)
// ------------------------------------------------
function renderConsumosChart() {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const label = new Date(Date.UTC(y2, m2 - 1, 1))
